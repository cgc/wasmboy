...
class WasmBoyLibService {
  ...
  async replaceRam(cartridgeRam) {
    await this.pause();
    // This sets IDB
    await WasmBoyMemory.saveCartridgeRam(WasmBoyMemory.cartridgeHeader, cartridgeRam);
    // This loads from IDB to set wasmboy
    await WasmBoyMemory.loadCartridgeRam();
    await this.reset();
  }

  clearMemoryRegion(start, end) {
    return WasmBoyMemory.clearMemoryRegion(start, end);
  }
